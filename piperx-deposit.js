...
async function piperx_IPdeposit(wallet){
    console.log(`IP测试币swap。。。。。。。。。。。。。。。。。。。。。`);
    const VerioIP=`0x6b2141D7b2bFD794C2c30e48A77cBC12ac1b4572`;
    //approve过程
    ...
    try {
        ...
        const token_address=`0x6b2141d7b2bfd794c2c30e48a77cbc12ac1b4572`;
        const amountTokenDesired=`10000000000000000`;
        const amountTokenMin=`9500000000000001`;
        const amountETHMin=`9689021697435388`;       
        const txResponse= contract.addLiquidityETH(token_address,amountTokenDesired,amountTokenMin,amountETHMin,wallet.address,Math.floor(Date.now()/1000)+360,{value:amountInBig});
...
const main=async(privateKeys)=>{
    ...
    await piperx_IPdeposit(morphl2_wallet) 
}
